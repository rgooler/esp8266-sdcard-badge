...
if (conf.wifi.mode == wifi.SOFTAP) or (conf.wifi.mode == wifi.STATIONAP) then
   ...
   conf.wifi.accessPoint.config.auth=wifi.OPEN
   conf.wifi.accessPoint.config.channel=11
   conf.wifi.accessPoint.config.ssid = "Jippen's Fileserver" -- Name of the WiFi network to create.
   -- conf.wifi.accessPoint.config.pwd = "ESP-"..node.chipid() -- WiFi password for joining - at least 8 characters
   conf.wifi.accessPoint.ip = {}
   conf.wifi.accessPoint.ip.ip = "192.168.1.1"
   conf.wifi.accessPoint.ip.netmask = "255.255.255.0"
   conf.wifi.accessPoint.ip.gateway = "192.168.1.1"
end
...
conf.mdns = {}
conf.mdns.hostname = 'fileserver' -- You will be able to access your server at "http://nodemcu.local."
conf.mdns.location = "Around Jippen's neck"
conf.mdns.description = "Jippen's esp8266 File Server"
